...
> Lead Agent 的综合判断（一家之言，不代表统一结论）。
> 回答：这个主题当前处于什么阶段？最核心的驱动力是什么？
> 每次更新时重写此节，保持最新。

## 各 Agent 观点（独立呈现）

> 每个 Agent 用自己的语言和分析框架，不被统一腔调。

### 技术视角（tech_generalist）

原文完整引用或详细摘要。保持原文的结论和语气。
...
| 长期(6m+) | ... | 6/10 | tech_gen | 2026-07-29 | 待验证 |

> 预测**不删除不修改**——只追加新预测或标记验证状态。
> 用于追踪各 Agent 的预测 accuracy，形成 accountability。

## 分歧地图

| 维度 | 观点 A | 观点 B | 分歧根因 | 状态 |
|------|--------|--------|---------|------|
| 估值 | 合理（Soros） | 偏高（Buffett） | 对增长持续性的假设不同 | ⚡未解决 |
| 入场时机 | 现在（Soros） | 等回调（Buffett） | 风险偏好差异 | ⚡未解决 |
...
| 日期 | 操作者 | 变更摘要 |
|------|--------|---------|
| 2026-08-01 | Lead | 集成 tech_scan 和宏观更新 |
| 2026-07-28 | Soros | 补充反身性分析 |
```

### 3.3 核心原则

1. **文档是世界观地图**——不是统一结论，是不同分析框架下的视角集合
2. **各 Agent 用自己的语言**——不被翻译成统一腔调，保持原文框架和结论
3. **预测不删不修改**——只追加或标记验证状态，形成 accountability
4. **分歧是核心产出**——分歧地图是必写章节，不是可选附录
5. **不强行拍板**——分歧标记"未解决"即可，等新数据出现再重新评估
