...
3. **分歧显式化**——不强行统一，分歧是核心信息，不是 bug
4. **不强行拍板**——分歧可标记为"未解决"，等新数据出现再评估
5. **来源可追溯**——每个观点标注来源 agent + 原文路径
...
3. Lead 整理：共识区 + 分歧区 + 未覆盖维度
4. 如有必要，Lead 发起追问/澄清（跨 2-3 轮）
5. Lead 判定：是否已形成可支撑写作的共识基础
   → 是 → 进入 Phase 2
   → 否 → 继续讨论或标记"讨论不足"，暂缓写作
```
