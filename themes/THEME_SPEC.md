...
Lead Agent 的多因子加权判断。
> 梳理主要因子与次要因子，标注各因子当前方向与置信度。
> 每次更新时重写此节，保持最新。

## 因子分析

按因子组织，每个因子一节。每个因子的结构：
...
**共识判断**：各 Agent 在此因子上的一致看法

**分歧**：如有，标注谁持不同看法及根因
        （不是所有因子都有分歧，没有就不写）

**Lead 评注**：此因子的置信度、边际变化方向、需关注什么
```

#### 示例

### 因子一：Blackwell 需求周期（权重：高）

**数据支撑**：强 — 供应链订单 QoQ +15%，多家机构上调 Q3 指引

**共识判断**：Blackwell 需求确定性高，产能爬坡超预期，数据中心收入将加速。

**分歧**：方向一致（Bullish），但 Soros 认为市场尚未充分定价（反身性上行空间），
Buffett 则认为当前 PE 45x 已包含乐观预期，上行空间有限。

**Lead 评注**：核心变量，分歧点在幅度而非方向。重点关注 Q3 财报的数据中心收入环比。

### 因子二：AI 产业链 capex 周期（权重：高）
...
| 因子 | 权重 | 方向 | 置信度 | 有无分歧 |
|------|------|------|--------|---------|
| Blackwell 需求 | 高 | Bullish | 高 | 幅度分歧 |
| AI capex 周期 | 高 | Bullish | 高 | 无 |
| 出口管制 | 中 | Bearish | 中 | 无 |
| 估值 | 中 | — | — | 有（合理 vs 偏高） |
...
| 时间窗 | 预测 | 置信度 | 提出者 | 验证 |
|--------|------|--------|--------|------|
| 短期(1-4w) | ... | 7/10 | Soros | 待验证 |
| 中期(1-3m) | ... | 5/10 | Buffett | 待验证 |
| 长期(6m+) | ... | 6/10 | Lead | 待验证 |
...
| 日期 | 操作者 | 变更摘要 |
|------|--------|---------|
| 2026-08-01 | Lead | 集成 Q3 指引上修，调整因子权重 |
```

### 3.3 核心原则 — 多因子模型思维

把 index.md 当作一份**多因子模型**来写：

1. **按因子组织，不按 Agent 组织**——每个因子是一节，不是每人写一章
2. **突出主要因子，不忽视次要因子**——权重影响篇幅，但不省略
3. **共识是因子主体**——先共识后分歧，分歧内联标注，不独立成篇
4. **因子总览表**——提供快速扫描所有因子的方向+置信度+分歧状态
5. **预测只追加不修改**——追踪各 Agent accuracy 和因子判断变化
...
Lead Agent 是**编辑/协调者**，不是主笔、不是决策者。
